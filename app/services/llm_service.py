...
class LLMService:
    ...
    def __init__(self, api_keys: Dict[str, str] = None):
        ...
        # 优先使用传入的API密钥，其次使用配置文件中的API密钥，最后使用环境变量
        self.api_keys = api_keys or {}
        
        # 从配置文件加载API密钥（如果未提供）
        if LLM_CONFIG and "API_KEYS" in LLM_CONFIG:
            for service, key in LLM_CONFIG["API_KEYS"].items():
                if service not in self.api_keys and key:
                    self.api_keys[service] = key
        
        # 从环境变量加载API密钥（如果配置文件中未提供）
